...
def test_auto_vt_book_path_math():
    """Book auto-VT with 1.5x cap: target_vol = 1 - pv,
    w_vol = min((1-pv)/pv, 1.5)."""
    ...
    # pv = 0.30 (low vol) -> raw 0.70/0.30 = 2.33 -> capped at 1.5 (levered)
    pv = 0.30
    cfg = VolTargetConfig(target_vol=auto_target_vol(pv), leverage_cap=1.5)
    ...
    assert abs(p["w_vol"] - 1.5) < 1e-9 and p["leverage_capped"] is True
    assert p["vt_auto"] is True and p["leverage_cap"] == 1.5
    assert abs(p["exposure"] - p["gate"] * 1.5) < 1e-9

    # pv = 0.60 (higher vol) -> target 0.40 -> w_vol = min(0.40/0.60, 1.5) = 0.667
    pv = 0.60
    cfg = VolTargetConfig(target_vol=auto_target_vol(pv), leverage_cap=1.5)
    p = hedge_parameters(close, rets, as_of=None, vt=40, config=cfg, rv_override=pv,
                         vol_source="portfolio", book_meta={"vt_auto": True, "leverage_cap": 1.5})
